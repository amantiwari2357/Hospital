...
const Profile = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                // Fetch full profile
                const profileRes = await fetch('http://localhost:5000/api/users/profile', config);
                ...
                const apptRes = await fetch(`http://localhost:5000/api/appointments?doctor=${encodeURIComponent(user?.name)}`, config);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5000/api/users/profile', config);
